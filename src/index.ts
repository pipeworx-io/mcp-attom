...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'ATTOM');
}
...
const tools: McpToolExport['tools'] = [
  ...
  },
  {
    ...
    description:
      '"When was [house] last sold" / "sale history of [property]" / "previous owners of [address]" / "what did [house] sell for last time" — complete sales/transaction history for a US property by address. Returns sale dates, prices, deed types, and buyer/seller details from public deed records via ATTOM.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        address1: { type: 'string', description: 'Street address (e.g., "123 Main St")' },
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object' as const,
      properties: {
        geoid: { type: 'string', description: 'A 5-digit ZIP code ("33158"). ATTOM\'s own "ZI"-prefixed form ("ZI33158") is also accepted; a bare ZIP is prefixed for you.' },
...
/**
 * ATTOM keys sales trends by its own geoid, which for a ZIP is just the ZIP with
 * a "ZI" prefix. An agent asked about "33158" has the ZIP and no reason to know
 * the prefix, so it sent the bare code and got an argument error for a call that
 * was one string concatenation away from working.
 */
function attomZipGeoid(raw: unknown): string {
  const v = String(raw ?? '').trim();
  return /^\d{5}$/.test(v) ? `ZI${v}` : v;
}
  ...
  switch (name) {
    ...
    case 'attom_sales_trend':
      return attomGet(key, '/salestrend/snapshot', {
        geoid: attomZipGeoid(args.geoid),
